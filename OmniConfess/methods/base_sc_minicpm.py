# -*- coding: utf-8 -*-
"""
base_sc_minicpm.py — MiniCPM-o-2.6 self-consistency (majority vote) baseline.

Runs model.chat() num_rollout times with sampling, then majority-votes
the parsed predictions.

Usage:
    CUDA_VISIBLE_DEVICES=1 python base_sc_minicpm.py \
        --datasets avhbench --data_path ./OmniHalluBench/avhbench/avh_converted.json \
        --output_dir ./results --file_name minicpm_sc_avh_v0 --max_samples 2
"""
...
from shared.eval_utils import make_result, save_result, parse_pred
...
def main():
    ...
    parser.add_argument('--file_name', type=str, default='minicpm_sc_out')
    ...
    for idx, item in enumerate(data):
        ...
        try:
            ...
            for s in range(args.num_rollout):
                if is_omni:
                    ...
                    roll_msgs = list(msgs)
                    if system_prompt:
                        roll_msgs.insert(0, {'role': 'system', 'content': [system_prompt]})
                    response = model.chat(
                        msgs=roll_msgs,
                        tokenizer=tokenizer,
                        sampling=True,
                ...
                print(f'  rollout {s+1}/{args.num_rollout}: {response[:80]!r}', flush=True)

            # Majority vote on parsed predictions
            parsed = [parse_pred(r, args.datasets) for r in rollout_responses]
            final_pred = majority_vote(parsed)

            elapsed = time.time() - t0
            print(f'  majority_vote => {final_pred[:120]!r}', flush=True)
        ...
        result = make_result(item, final_pred, args.datasets, 'minicpm_sc',
                             time_sec=elapsed,
                             cot=json.dumps(rollout_responses))
